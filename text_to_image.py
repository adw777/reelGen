...
#                     # Try to enable xformers only if on GPU
#                     try:
...
#                 # Set up generator with appropriate device
#                 if seed is not None:
#                     generator = torch.Generator(device=self.device).manual_seed(seed + attempt)
#                 else:
#                     generator = torch.Generator(device=self.device)

#                 # Adjust inference parameters based on device
#                 inference_steps = 30 if self.device == "cpu" else 50
